...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def mask(self, x, src_img_features, lay_idx):
        ...
        mask_img1 = mask_img.shape[0]
        mask_img2 = mask_img.shape[1]
        mask_img3 = mask_img.shape[2]
        x1 = x.shape[0]
        x2 = x.shape[1]
        x3 = x.shape[2]
        src_img_features1 = src_img_features.shape[0]
        src_img_features2 = src_img_features.shape[1]
        src_img_features3 = src_img_features.shape[2]
        mask_img = mask_img.cuda().reshape(-1,x2)
        #T = x.cpu().reshape(-1,128)
        lambd = 0.1

       
        cost_mat = mask_img
        num_fg_pix = (src_img_features.cpu().detach().reshape(-1,128)).shape[0]
        #num_fg_pix = mask_img.shape[0]
        num_text = x2
        ...
        mask_img = ot.sinkhorn(b, a, cost_mat, lambd, numItermax = 100)
        mask_img = mask_img.view(mask_img1,mask_img2,mask_img3)

        #train float16; test float132
        mask_img = mask_img.to(torch.float16)
